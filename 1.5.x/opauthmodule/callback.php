...
if (isset($_POST['email_opthmodule'])) {
    ...
    if (!$_customer_id_exits) {
        ...
        $sql = 'INSERT INTO `' . _DB_PREFIX_ . 'customer` 
        		SET id_gender = ' . $_POST['gender'] . ', 
        ...
        $sql = 'INSERT INTO `' . _DB_PREFIX_ . 'customer_group` 
        		SET id_customer = ' . $insert_id . ', 
        			id_group = ' . $id_group . ' ';
        ...
        INSERT INTO  `' . _DB_PREFIX_ . 'customer_opauth_' . strtolower($_POST['provider']) . '` (`' . strtolower($_POST['provider']) . '_id`,`user_id`) 
    ...
    Db::getInstance()-> Execute('
    	...
    	SET active = 0, 
    		act_key = "' . $actkey . '" WHERE id_customer = "' . $insert_id . '"');

    if (!Mail::Send((int)$cookie->id_lang, 'opauthmodule', Mail::l('Welcome!', (int)
        $cookie->id_lang), array(
        	'{firstname}'  => Tools::ucfirst(html_entity_decode($_POST['firstname'], ENT_COMPAT, "UTF-8")),
        	'{lastname}'   => Tools::strtoupper(html_entity_decode($_POST['lastname'], ENT_COMPAT, "UTF-8")),
        	'{email}'      => $_POST['email_opthmodule'],
        	'{passwd}'     => $real_passwd,
        	'{actlink}'    => $actlink), 
...
function createUser($_data)
{
    ...
    SELECT * FROM ' ._DB_PREFIX_ . 'customer_opauth_' . strtolower($_data['provider']) . '
    ...
    if (sizeof($result1)) {
        ...
        if (isset($_data['namePerson/first']) && isset($_data['namePerson/last'])) {
            $firstname  = pSQL(Tools::ucfirst($_data['namePerson/first']));
            $lastname   = pSQL(Tools::strtoupper($_data['namePerson/last']));
        }
        ...
        if ($_customer_id_exits) {
            ...
            $insert = Db::getInstance()-> Execute('
            INSERT INTO  `' . _DB_PREFIX_ . 'customer_opauth_' . strtolower($_data['provider']) . '` (`' . strtolower($_data['provider']) . '_id`,`user_id`) 
            ...
            if ($email == "nothing") {
                ...
                echo '
                	<a id="add_email" href="'.$opauth->url.'/modules/opauthmodule/email.php?gender=' . $gender . '&firstname=' . $firstname . '&lastname=' . $lastname . '&provider=' . $_data['provider'] . '&idu=' . $_data['id'] . '"></a>';
            }
            if ($email != "nothing") {
                if (!isset($_POST['email_opthmodule'])) {
                ...
                if (!$authentication or !$customer->id) {
                    ...
                    Mail::Send(intval($cookie->id_lang), 'account', 'Welcome!', array(
                        '{firstname}'   => Tools::ucfirst($customer->firstname),
