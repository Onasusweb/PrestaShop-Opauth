...
class LinkedInStrategy extends OpauthStrategy{
	...
	public $defaults = array(
		...
		'profile_fields' => array('id', 'first-name', 'last-name', 'formatted-name', 'headline', 'picture-url', 'summary', 'location', 'public-profile-url', 'site-standard-profile-request', 'email-address', 'date-of-birth'),
		...
		if ($_REQUEST['oauth_token'] == $session['oauth_token']){
			...
			if ($results !== false && !empty($results['oauth_token']) && !empty($results['oauth_token_secret'])){
				...
				if (!empty($profile['id'])){
					...
					$this->mapProfile($profile, 'date-of-birth', 'info.birthday');
